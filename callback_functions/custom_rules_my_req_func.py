...
def get_request_id_for_user(user_id : int = None) -> list[int] :
    if user_id is None:
        sql_query = 'select request_id from custom_rules_request order by request_id'
    else:
...
def refresh_request_id(url):
    if url == main_app.environment_details["custom_rules_my_req"]:
        return get_request_id_for_user()
...
@main_app.app.callback(
    ...
    State("my_req_requested_user","value"),
    State("my_req_user_ids","value"),
    prevent_initial_call = True
)
def update_comments_section(n_clicks , existing_comments,new_comment,comment_by,request_id):
...
def close_ticket(n_clicks ,request_id,exisitng_status):
    ...
    if exisitng_status == 'Closed':
        return no_update, "Ticket already Closed" ,"Warning!","warning",2500,True
    ...
    if status_updated != 'Error':
        return  "Closed", "Ticket Closed" ,"Info!","info",2500,True
    else:
